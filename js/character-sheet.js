...
window.BB_CHARACTER_SHEET = (() => {
  let activeTab = localStorage.getItem("bb_sheet_tab") || "spells";
  ...
  function actualRender() {
    ...
    allConditions.forEach(c => {
      if (char.conditions[c]) {
        activeConditionBadges += `<span class="condition-badge"><i class="fas fa-skull"></i> ${c}</span>`;
      }
    });
    if (char.conditions.exhaustion > 0) {
      activeConditionBadges += `<span class="condition-badge" style="color:#EF4444; border-color:#EF4444;"><i class="fas fa-lungs-virus"></i> Exhaustion ${char.conditions.exhaustion}</span>`;
    }
    ...
    try {
      ...
      container.innerHTML = `
        <div class="sheet-page glass" style="display:flex; flex-direction:column; gap:20px;">
          <!-- Main Row -->
          <div class="sheet-main-row" style="display:flex; flex-wrap:wrap; gap:20px; align-items:stretch;">
            
            <!-- COLUMN 1: Profile & Vitals (Formerly Top Bar) -->
            <div class="sheet-col col-vitals glass" style="flex: 0 0 320px; display:flex; flex-direction:column; gap:15px; padding:15px; border-radius:12px; overflow-y:auto; overflow-x:hidden;">
              ${topBarHtml}
            </div>
            ...
            <div class="sheet-col col-details glass" style="flex: 1; min-width: 300px; display: flex; flex-direction: column;">
              <div class="details-tab-bar" style="flex-shrink: 0; border-bottom:1px solid rgba(255,255,255,0.1);">
                <button class="sheet-tab-btn ${activeTab === "spells" ? "active" : ""}" data-tab="spells">Combat</button>
  ...
  function renderTopBar(char, selectOptions) {
    ...
    const availableRestDice = char.restDice !== undefined ? char.restDice : char.level;
    const maxRestDice = char.level;
    ...
    const renderAttrBox = (statKey, statName) => {
        ...
        return `
          <div class="attr-box stat-roll-hook glass info-tooltip-trigger" style="display:flex; flex-direction:column; align-items:center; padding:5px 10px; min-width:50px; border:1px solid rgba(255,193,7,0.3); border-radius:8px; cursor:pointer;" data-type="attribute" data-name="${statName}" data-label="${statName}" data-mod="${mod}">
    ...
    return `
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          ...
          <div style="position:relative; flex:1;">
            ...
            <div id="short-rest-panel" style="display:none; position:absolute; top:110%; right:0; width:160px; flex-direction:column; gap:6px; padding:8px; border:1px solid var(--amber); border-radius:6px; background:var(--bg-dark); z-index:10000; box-shadow:0 4px 10px rgba(0,0,0,0.8);">
              <span style="font-size:0.75rem; color:#fff; text-align:center;">Spend d${restDieSize} Rest Die: <span style="color:var(--amber);">${availableRestDice} / ${maxRestDice}</span></span>
              ...
              <button class="btn btn-xs btn-brown" id="btn-short-rest">Complete Rest (No Dice)</button>
            </div>
          </div>
      ...
      <div style="display:flex; gap:10px; align-items:center; justify-content:center;">
        <div class="def-box glass info-tooltip-trigger" data-type="attribute" data-name="Defense" style="padding:5px 10px; text-align:center; min-width:50px;">
        ...
        <div class="def-box glass info-tooltip-trigger" data-type="attribute" data-name="Resilience" style="padding:5px 10px; text-align:center; min-width:50px;">
        ...
        <div class="def-box glass info-tooltip-trigger" data-type="attribute" data-name="Speed" style="padding:5px 10px; text-align:center; min-width:50px;">
      ...
      <div style="display:flex; flex-direction:column; gap:5px;">
        <div style="display:flex; gap:10px; justify-content:center; flex-wrap:wrap;">
            <div class="resource-col glass" style="display:flex; flex-direction:column; align-items:center; padding:5px 10px; border-radius:8px;">
                <div style="font-size:0.75rem; color:var(--text-light); font-weight:bold;">HP <span style="color:#aaa; font-size:0.6rem; font-weight:normal;">Temp: <input type="number" class="inline-hp-input" id="hp-temp" value="${char.hp.temp || 0}" style="width:25px; padding:0; text-align:center;"></span></div>
                <div style="display:flex; align-items:center; margin-top:2px;">
                    <input type="number" class="res-val-input" id="hp-current" value="${char.hp.current}" style="width:40px; font-size:1rem; padding:2px; text-align:center;">
                    <span style="margin:0 4px; color:#666;">/</span>
                    <input type="number" class="res-val-input" id="hp-total" value="${char.hp.total}" readonly style="width:40px; font-size:1rem; padding:2px; color:#aaa; text-align:center;">
                </div>
            </div>
            <div class="resource-col glass" style="display:flex; flex-direction:column; align-items:center; padding:5px 10px; border-radius:8px;">
                <div style="font-size:0.75rem; color:var(--text-light); font-weight:bold;">SP <span style="color:#aaa; font-size:0.6rem; font-weight:normal;">Temp: <input type="number" class="inline-hp-input" id="sp-temp" value="${char.sp?.temp || 0}" style="width:25px; padding:0; text-align:center;"></span></div>
                <div style="display:flex; align-items:center; margin-top:2px;">
                    <input type="number" class="res-val-input" id="sp-current" value="${char.sp.current}" style="width:40px; font-size:1rem; padding:2px; text-align:center;">
                    <span style="margin:0 4px; color:#666;">/</span>
                    <input type="number" class="res-val-input" id="sp-total" value="${char.sp.total}" readonly style="width:40px; font-size:1rem; padding:2px; color:#aaa; text-align:center;">
                </div>
            </div>
            <div class="resource-col glass" style="display:flex; flex-direction:column; align-items:center; padding:5px 10px; border-radius:8px;">
                <div style="font-size:0.75rem; color:var(--text-light); font-weight:bold;">MP <span style="color:#aaa; font-size:0.6rem; font-weight:normal;">Temp: <input type="number" class="inline-hp-input" id="mp-temp" value="${char.mp?.temp || 0}" style="width:25px; padding:0; text-align:center;"></span></div>
                <div style="display:flex; align-items:center; margin-top:2px;">
                    <input type="number" class="res-val-input" id="mp-current" value="${char.mp.current}" style="width:40px; font-size:1rem; padding:2px; text-align:center;">
                    <span style="margin:0 4px; color:#666;">/</span>
                    <input type="number" class="res-val-input" id="mp-total" value="${char.mp.total}" readonly style="width:40px; font-size:1rem; padding:2px; color:#aaa; text-align:center;">
                </div>
            </div>
        </div>
      </div>
      ...
      <div style="display:flex; flex-direction:column; gap:5px; min-width:150px; flex:1;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <strong style="font-size:0.75rem; color:var(--text-light);">Conditions <button id="open-conditions-btn" style="background:transparent; border:none; color:var(--amber); cursor:pointer; font-weight:bold;">+</button></strong>
            <div style="display:flex; gap:2px;">${activeConditionBadges}</div>
        </div>
        <div style="display:flex; justify-content:space-between; align-items:center;">
      ...
      <div id="conditions-flyout" class="conditions-flyout-panel glass" style="display:none; position:absolute; right:20px; top:80px; z-index:10000; width:250px; max-height:400px; overflow-y:auto;">
        <h3 style="margin-top:0; border-bottom:1px solid var(--amber); padding-bottom:5px; margin-bottom:10px;">Conditions</h3>
        <div class="conditions-list">
          ${allConditions.map(c => {
            ...
            <div class="condition-toggle-row">
              ...
              <label class="switch">
                <input type="checkbox" class="condition-toggle-cb" data-condition="${c}" ${char.conditions[c] ? 'checked' : ''}>
        ...
        <div class="exhaustion-tracker">
          <div style="display:flex; justify-content:space-between; align-items:flex-end;">
            ...
            <span style="font-size:0.75rem; color:var(--text-light);">Level ${char.conditions.exhaustion || 0}</span>
          </div>
          <div class="exhaustion-levels">
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) === 0 ? 'active' : ''}" data-level="0" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>No exhaustion.</p>">--</button>
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) >= 1 ? 'active' : ''}" data-level="1" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>1. Disadvantage on ability checks.</p>">1</button>
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) >= 2 ? 'active' : ''}" data-level="2" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>2. Stamina costs are doubled.</p>">2</button>
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) >= 3 ? 'active' : ''}" data-level="3" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>3. Disadvantage on attacks and saving throws.</p>">3</button>
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) >= 4 ? 'active' : ''}" data-level="4" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>4. Hit point maximum is halved.</p>">4</button>
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) >= 5 ? 'active' : ''}" data-level="5" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>5. Speed becomes 0, and the creature can&#39;t move.</p>">5</button>
            <button class="exhaustion-level-btn info-tooltip-trigger ${(char.conditions.exhaustion || 0) >= 6 ? 'active' : ''}" data-level="6" style="border-color:#EF4444; color:#EF4444;" data-html="<p style='margin:0; font-size:0.85rem; color:#fff;'>6. Death.</p>">6</button>
          </div>
        </div>
      </div>
    `;
  ...
  function renderSkillsColumn(char) {
      ...
      skillsList.forEach(sk => {
        ...
        skillsHTML += `
          <div class="skill-item stat-roll-hook glass hover-lift info-tooltip-trigger" style="padding: 1px 6px; margin: 0; display: flex; justify-content: space-between; align-items: center; border-radius: 3px; border: 1px solid rgba(255,255,255,0.05); cursor:pointer;" data-type="skill" data-name="${sk.name}" data-label="${sk.name}" data-mod="${totalMod}" data-html="<h4>${sk.name} Breakdown</h4><p style='margin:0'>${breakdownHtml.replace(/"/g, '&quot;')}</p><hr style='border-color: rgba(255,255,255,0.1); margin: 8px 0;'><p style='font-size:0.85rem; color:#fff; margin:0;'>${(window.BB_DATABASE.SKILL_DESCRIPTIONS[sk.name] || '').replace(/"/g, '&quot;')}</p>">
            <div class="skill-label" style="display:flex; align-items:center; gap:6px; overflow:hidden;">
              <span class="skill-name" style="font-weight:bold; color:#fff; font-size:0.7rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${sk.name}</span>
              <span class="skill-attr" style="color:var(--text-light); font-size:0.55rem; white-space:nowrap;">(${sk.attr})</span>
            </div>
            <span class="skill-mod-display" style="color:var(--amber); font-weight:bold; font-size:0.75rem; white-space:nowrap;">${skillModStr}</span>
          </div>
        `;
      });
      ...
      return `
        <div style="flex:0 0 350px; display:flex; flex-direction:column; gap:10px; height: 100%;">
            <div class="card glass" style="flex:1; display:flex; flex-direction:column; padding:15px; border-radius:8px; border:1px solid rgba(255,255,255,0.1); overflow:hidden;">
              <h3 class="card-header-sm" style="margin-bottom:10px;">
                <span class="roll-initiative-btn info-tooltip-trigger" style="cursor:pointer; border-bottom:1px dashed var(--amber); transition:color 0.2s;" onmouseover="this.style.color='var(--amber)'" onmouseout="this.style.color=''" data-html="<h4>Initiative</h4><p style='margin:0; font-size:0.85rem; color:#fff;'>Roll 1d20 + your Dexterity modifier to determine turn order in combat.</p>">Skills (Roll Init)</span>
              </h3>
              <div class="skills-list-container" style="flex:1; display:grid; grid-template-columns: 1fr; grid-auto-rows: max-content; gap:2px; overflow:hidden;">
                ${skillsHTML}
              </div>
  ...
  function renderTabContent(char) {
    ...
    if (activeTab === "equipment") {
      ...
      return `
        <div style="display:flex; gap:20px; justify-content:center; align-items:stretch; flex:1; overflow:hidden;">
          ...
          <div style="flex:0 0 350px; display:flex; flex-direction:column;">
            ...
            <div class="card combat-stats-card glass" style="flex:1; padding:15px; margin:0;">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:1px solid rgba(255,193,7,0.3); padding-bottom:8px;">
                <h3 class="card-header-sm" style="margin:0; border:none; padding:0;">Combat</h3>
              ...
              <div class="attacks-list" style="display:flex; flex-direction:column; gap:8px;">
                ${(function() {
                  ...
                  if (!hasWeaponEquipped || char.class === "Disciple") {
                    let strMod = window.BB_STATE.getModifier(char.stats.Str);
                    attacksHtml += `<button class="btn btn-secondary attack-roll-btn" data-slot="unarmed" data-grip="" data-label="Damage: Unarmed Strike" data-count="1" data-type="1" data-mod="${strMod}" data-crit="0" data-bowmens="0" style="display:flex; justify-content:space-between; align-items:center; width:100%; padding:8px 12px; font-family:var(--font-mono); font-size:0.85rem; text-align:left; border:1px solid rgba(255,255,255,0.1); background:rgba(0,0,0,0.3); cursor:pointer;" title="Roll 1 Bludgeoning ${strMod >= 0 ? '+'+strMod : strMod} (Crit Range: 0)"><span>Unarmed Strike <span style="color:#fff; font-size:0.75rem;">(Body)</span></span><span style="color:var(--amber); display:flex; align-items:center;">1 ${strMod >= 0 ? '+'+strMod : strMod} Bludgeoning <span style="font-size:0.7rem; color:#fff; margin-left:4px;">(STR)</span></span></button>`;
                  }
  ...
  function setupEventListeners(char) {
    ...
    // Short Rest (no automatic recovery without rest dice)
    ...
    const updateConditionBadges = () => {
      ...
      allConditions.forEach(c => {
        if (char.conditions[c]) {
          badges += `<span class="condition-badge"><i class="fas fa-skull"></i> ${c}</span>`;
        }
      });
      if (char.conditions.exhaustion > 0) {
        badges += `<span class="condition-badge" style="color:#EF4444; border-color:#EF4444;"><i class="fas fa-lungs-virus"></i> Exhaustion ${char.conditions.exhaustion}</span>`;
      }
    ...
    if (openConditionsBtn && conditionsFlyout) {
      openConditionsBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        conditionsFlyout.style.display = conditionsFlyout.style.display === "none" ? "flex" : "none";
      });

      document.addEventListener("click", (e) => {
        if (!conditionsFlyout.contains(e.target) && e.target !== openConditionsBtn) {
    ...
    if (customRollBtn && customRollInput) {
      customRollBtn.addEventListener("click", () => {
        ...
        if (match) {
          ...
          window.BB_DICE.roll(`Custom (${input})`, count, type, mod, 0, 0, false);
          setTimeout(() => { if(typeof renderRollLog === "function") renderRollLog(); }, 100);
        } else {
          if(window.BB_DICE) window.BB_DICE.showToastNotification("Invalid format. Use XdY+Z (e.g., 2d6+4)");
        }
      });
    ...
    const handleRestDieConsume = (pool) => {
        if (!char.restDice) char.restDice = { used: 0 };
        if (char.restDice.used >= char.level) return false; // No dice left
        ...
        let rollResult = Math.floor(Math.random() * restDieSize) + 1;
        let isTroll = char.ancestry === "Troll";
        if (isTroll) {
          const roll2 = Math.floor(Math.random() * restDieSize) + 1;
        ...
        // Log it
        let breakdownText = `Roll: [${rollResult}${isTroll ? ' (Advantage)' : ''}] ${baseMod >= 0 ? '+' : '-'} ${Math.abs(baseMod)} (${statName})`;
        if (hasRestful) breakdownText += ` + 1 (Restful)`;
        
        return { poolName, restDieSize, mod, totalRestored, breakdownText };
    };

    document.querySelectorAll(".short-rest-option").forEach(btn => {
      btn.addEventListener("click", (e) => {
        const pool = btn.getAttribute("data-pool");
        const res = handleRestDieConsume(pool);
        if (res) {
          window.BB_STATE.addDiceRoll(`Short Rest (${res.poolName})`, 1, res.restDieSize, res.mod, res.totalRestored, res.breakdownText);
          window.BB_STATE.saveCharacter(char);
          window.BB_DICE.showToastNotification(`Restored ${res.totalRestored} ${res.poolName}!`);
          render(); // Update UI
        }
      });
    });
    ...
    document.querySelectorAll(".use-potion-btn").forEach(btn => {
      btn.addEventListener("click", (e) => {
        ...
        if (index > -1) {
          ...
          const res = handleRestDieConsume(pool);
          if (res) {
            window.BB_STATE.addDiceRoll(`Used ${potionName}`, 1, res.restDieSize, res.mod, res.totalRestored, res.breakdownText);
            window.BB_STATE.saveCharacter(char);
            window.BB_DICE.showToastNotification(`Used ${potionName}, restored ${res.totalRestored} ${res.poolName}!`);
            render(); // Update UI
          }
        }
      });
