    ...
    results.forEach(res => {
      ...
      card.innerHTML = `
        ...
        <h3 class="card-title">${res.name}</h3>
          <div class="card-meta">${res.category === 'spell' ? `${res.type ? `Type: ${res.type} | ` : ''}Cost: ${res.cost} | Activation: ${res.actTime} | Range: ${res.range} | Components: ${res.components} | Duration: ${res.duration} | Class: ${res.class}` : res.category === 'item' ? `Rarity: ${res.rarity || 'Common'}${res.type ? ` | Type: ${res.type}` : ''}${res.training ? ` | Training: ${res.training}` : ''}${res.class ? ` | Class: ${res.class}` : ''}${res.slot ? ` | Slot: ${res.slot}` : ''}` : res.category === 'misc' ? `Type: ${res.type || 'Item'}${res.slot ? ` | Slot: ${res.slot}` : ''}` : res.category === 'monster' ? `Level ${res.level || Math.round(((res.stats?.Str||10)+(res.stats?.Dex||10)+(res.stats?.Con||10)+(res.stats?.Int||10)+(res.stats?.Wis||10)+(res.stats?.Lck||10))/3)} | DR ${res.difficultyRating || Math.round(Math.round(((res.stats?.Str||10)+(res.stats?.Dex||10)+(res.stats?.Con||10)+(res.stats?.Int||10)+(res.stats?.Wis||10)+(res.stats?.Lck||10))/3)/3)} | ${res.type}` : (res.category === 'feat' || res.category === 'talent') ? `Requirement: ${res.requirement}` : ''}</div>
